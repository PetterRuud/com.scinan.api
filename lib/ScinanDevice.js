...
class ScinanDevice extends Homey.Device {
  async onInit() {
    ...
    const updateInterval = this.getSetting('interval');
    this.log('updateInterval', updateInterval);

    try {
      const task = await Homey.ManagerCron.getTask(cronName);
  ...
  async getDeviceData() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        //self.setCapabilityValue('measure_power', measure_power);
  ...
  onDeleted(name) {
    ...
    ManagerCron.unregisterTask('refreshdevices');
  }
}
